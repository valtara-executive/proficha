...
export default function ProFicha() {
  ...
  const connectDrive = () => {
    showToast(lang === "es" ? "Redirigiendo a Google Drive OAuth..." : "Redirecting to Google Drive OAuth...", "info");
    setTimeout(() => { setDriveConnected(true); storage.set("driveConnected", true); showToast(t.driveConnected); }, 2000);
  };
  ...
  const RecordsView = () => {
    ...
    const createRecord = () => {
      ...
      const r = { id: genId(), patientName: newName.trim(), sectorId: newSector, date: new Date().toLocaleDateString(lang === "es" ? "es-MX" : "en-US"), dateKey: todayKey(), ts: Date.now() };
      setRecords(prev => [r, ...prev]);
      setNewName(""); setShowNew(false);
      showToast(lang === "es" ? "Expediente creado ✓" : "Record created ✓");
    };
      ...
      <div>
        ...
        {showNew && (
          <div className="pf-card" style={{ marginBottom: 20, borderColor: pal.accent + "44" }}>
            ...
            <input className="pf-input" value={newName} onChange={e => setNewName(e.target.value)} placeholder={lang === "es" ? "Nombre completo..." : "Full name..."} onKeyDown={e => e.key === "Enter" && createRecord()} autoFocus />
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 18 }}>
              <button className="pf-btn pf-btn-primary" onClick={createRecord}><Icon name="check" size={18} color="inherit" /> {lang === "es" ? "Crear Expediente" : "Create Record"}</button>
          ...
          <header className="pf-topbar">
            ...
            <div className="pf-topbar-right">
              ...
              <div className="pf-avatar" onClick={() => navigate("settings")} title={t.profile}>
                {profile.logo ? <img src={profile.logo} alt="logo" /> : avatarChar}
              </div>
            </div>
          </header>
